...
This registers `GET /qpay/webhooks` as the webhook endpoint.
...
The engine registers a webhook controller at `GET /qpay/webhooks` (via the engine mount).

### Webhook Flow

When QPay sends a callback:

1. The `WebhooksController` extracts `qpay_payment_id` from the request query parameters
...
4. Returns a plain text `SUCCESS` response
...
ActiveSupport::Notifications.subscribe("payment_received.qpay") do |name, start, finish, id, payload|
  qpay_payment_id = payload[:qpay_payment_id]
  result = payload[:result]

  # Update order status
  order = Order.find_by(qpay_payment_id: qpay_payment_id)
...
class PaymentSubscriber < ActiveSupport::Subscriber
  ...
  def payment_received(event)
    qpay_payment_id = event.payload[:qpay_payment_id]
    result = event.payload[:result]

    Order.find_by(qpay_payment_id: qpay_payment_id)&.update(status: :paid)
  end
end
```
...
ActiveSupport::Notifications.subscribe("payment_received.qpay") do |*args|
  ...
  Rails.logger.info "QPay payment received: #{event.payload[:qpay_payment_id]}"
end
```
...
RSpec.describe "QPay Webhook" do
  it "fires payment_received notification" do
    ...
    get "/qpay/webhooks", params: { qpay_payment_id: "pay_123" }

    expect(events.length).to eq(1)
    expect(events.first.payload[:qpay_payment_id]).to eq("pay_123")
  end
end
```
...
|------------|-------------|-------------|
| `payment_received.qpay` | `:qpay_payment_id`, `:result` | Fired when payment is confirmed |
