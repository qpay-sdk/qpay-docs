...
The middleware intercepts `GET` requests to the configured `WebhookPath` (default: `/api/qpay/webhook`).

### Webhook Flow

When QPay sends a callback:

1. The middleware intercepts the GET request at the webhook path
2. Extracts `qpay_payment_id` from the query parameters (`context.Request.Query["qpay_payment_id"]`)
...
5. Returns plain text `"SUCCESS"` response
...
public class WebhookTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    [Fact]
    public async Task Webhook_ReturnsSuccess()
    {
        var response = await _client.GetAsync("/api/qpay/webhook?qpay_payment_id=pay_123");
