...
- **Maven Central**: [io.github.qpay-sdk:qpay-spring-boot-starter](https://central.sonatype.com/artifact/io.github.qpay-sdk/qpay-spring-boot-starter)
...
The starter includes a built-in `QPayWebhookController` that handles `GET` requests at the configured webhook path (default: `/qpay/webhook`).

When QPay sends a callback:

1. The controller extracts `qpay_payment_id` from the query parameter
...
```java
@GetMapping("${qpay.webhook-path:/qpay/webhook}")
```
