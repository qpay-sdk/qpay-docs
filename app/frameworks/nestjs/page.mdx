...
The package includes a `QPayWebhookController` that handles `GET /qpay/webhook`:
...
The controller:
1. Accepts `GET /qpay/webhook?qpay_payment_id=...` with the payment ID as a query parameter
2. Calls `checkPayment` to verify the payment
3. Returns plain text `SUCCESS` after verification
...
export class CustomWebhookController {
  ...
  async handleWebhook(@Query('qpay_payment_id') paymentId: string) {
    if (!paymentId) {
      return 'ERROR';
    }
    ...
    if (result.rows?.length > 0) {
      this.eventEmitter.emit('payment.received', { paymentId, result });
    }
...
describe('QPayWebhookController', () => {
  ...
  it('should return SUCCESS', async () => {
    const result = await controller.handleWebhook('pay_123');
