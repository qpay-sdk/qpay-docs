...
This registers the webhook endpoint at `GET /qpay/webhook/`.
...
The package includes a built-in webhook view at `GET /qpay/webhook/` (when you include the URLs as shown above). The CSRF exemption is handled automatically.

When QPay sends a callback:

1. The view reads the `qpay_payment_id` from the query parameters
...
|------|-----|--------|-------------|
| `WebhookView` | `/webhook/` | GET | Webhook callback handler |
