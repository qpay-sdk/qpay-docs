...
- **Version**: 1.0.0
- **License**: MIT
- **Marketplace**: WordPress.org (submitted, awaiting review)
...
5. **Confirmation**: Payment is confirmed via two parallel mechanisms:
   ...
   - **Server-side webhook**: QPay sends a GET request to the callback URL with `qpay_payment_id` as a query parameter. The webhook handler verifies the payment and calls `$order->payment_complete()`
...
1. Reads the `qpay_payment_id` from the query parameter
...
- Verify there are no security plugins blocking GET requests to the webhook endpoint
