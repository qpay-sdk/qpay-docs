...
6. **Webhook Callback**: QPay also sends a GET request to the callback URL (`extension/payment/qpay.callback`) with `qpay_payment_id` as a query parameter
...
The callback endpoint receives a GET request from QPay with `qpay_payment_id` as a query parameter. It uses this payment ID to verify the payment status and returns a JSON acknowledgment.

Make sure the callback URL is publicly accessible. For OpenCart installations behind a reverse proxy, ensure the proxy passes GET requests to the callback route.
...
- Test the callback URL manually with a GET request: `?qpay_payment_id=test`
- Check for mod_security or firewall rules blocking callback requests
