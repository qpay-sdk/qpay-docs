...
6. **Webhook**: QPay also sends a server-side callback to `GET /webhook/qpay?qpay_payment_id=<id>` for reliable confirmation
...
| `GET` | `/webhook/qpay` | Receives QPay webhook callbacks (via `qpay_payment_id` query parameter) |
...
The webhook handler receives a `qpay_payment_id` query parameter, verifies the payment via `POST /v2/payment/check`, and returns the payment status.
